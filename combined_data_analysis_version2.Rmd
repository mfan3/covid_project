...
mobility.active.bottom <- mobility.active %>%
  ...
  group_by(state, biweek) %>%
  summarise(active.mean = active.avg,
            mobility.avg = mean(c(retail.recreation.avg, grocery.pharmacy.avg, parks.avg, transit.avg, workplace.avg, residential.avg)))
...
# residential 
workplace.active.top <- lm(workplace.mean ~ mobility.mean, data = mobility.active.top)
workplace.res <- residuals(workplace.active.top)

workplace.sum <- summary(lm(workplace.res ~ residuals(retail.active.top)))
