...
sum(is.na(mobility.change1$parks.diff))
...
mobility.active <- mobility.active %>%
  ...
  select(c(state, date, retail.recration.diff, grocery.pharmacy.diff, parks.diff, transit.diff, workplaces.diff, residential.diff, active.changes)) %>% 
  filter(date >= as.Date("2020-04-13")) %>% 
  mutate(month = months(date)) %>%
  group_by(state, month) %>%
  summarise(active.avg = mean(active.changes),
            retail.recreation.avg = mean(retail.recration.diff),
            grocery.pharmacy.avg = mean(grocery.pharmacy.diff), 
            parks.avg = mean(parks.diff, na.rm=TRUE), 
            transit.avg = mean(transit.diff, na.rm=TRUE),
            workplace.avg = mean(workplaces.diff),
            residential.avg = mean(residential.diff)) 
