...
```{r, message=FALSE}
covid.confirmed.raw <- read_.csv("./datasets/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv")
...
mobility.active.rates <- mobility.active.rates %>%
  ...
  filter(date >= as.Date("2020-04-13")) %>%
  group_by(state) %>%
  mutate(avg.3days = (rollsumr(active.rates, k = 3, fill = NA))/3,
         avg.7days = (rollsumr(active.rates, k = 7, fill = NA))/7)
...
mobility.active.rates.summary <-  mobility.active.rates %>% 
  mutate(month = months(date),
         week = format(date, format = '%W'),
         # the following code converts to biweekly
         biweek = 14 * (as.numeric(date - min(date)) %/% 14) + min(date)) %>%
