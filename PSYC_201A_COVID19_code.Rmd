...
mobility.active.rates$avg.3days = (rollsumr(mobility.active.rates$active.rates, k = 3, fill = NA))/3
mobility.active.rates$avg.7days = (rollsumr(mobility.active.rates$active.rates, k = 7, fill = NA))/7
...
mobility.pca <- prcomp(mobility.active.comp$completeObs, center = TRUE, scale = TRUE)
#mobility.pca$x[,"PC1"]
mobility.pca$rotation <- -mobility.pca$rotation
mobility.pca$x <- -mobility.pca$x
...
# add PC1 as pca
mobility.active.rates.imputed$mobility.pca = mobility.pca$x[,"PC1"]

```

biweek binning of dates
```{r}
# filter dates, bin data biweek
mobility.active.rates.imputed %>% 
  ...
  mutate(month = months(date),
         ...
         biweek = 14 * (as.numeric(date - min(date)) %/% 14) + min(date)) %>%
  group_by(state, biweek) %>%
  summarise(retail.recreation.avg = mean(retail_and_recreation_percent_change_from_baseline),
            grocery.pharmacy.avg = mean(grocery_and_pharmacy_percent_change_from_baseline),
            parks.avg = mean(parks_percent_change_from_baseline),
            transit.avg = mean(transit_stations_percent_change_from_baseline),
            workplace.avg = mean(workplaces_percent_change_from_baseline),
            active.avg = mean(active.rates),
            growth.rate.avg = mean(growth.rate),
            mobility.pca.avg = mean(mobility.pca))
```

### previous codes for partial regression (NEED TO BE MODIFIED)
```{r}
# do we need average for all states?
mobility.active.bottom <- mobility.active %>%
  filter(state %in% svi.pop.bottom.states) %>%
  group_by(state, biweek) %>%
  summarise(active.mean = active.avg,
