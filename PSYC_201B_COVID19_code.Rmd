...
# dim(covid.mob.svi) # 1311362, 11
# filter out large population counties 
# active rates df
covid.mob.svi <- covid.mob.svi %>%
  ...
  filter(date >= as.Date("2020-04-13") & date <= as.Date("2020-12-31")) %>%
...
covid.time.series <- covid.time.series %>%
  ...
  group_by(county) %>%
  mutate(growth.rate.lag = lag(growth.rate, 11)) %>% # 2020-04-26 for one week lag
...
covid.time.series.nas <- covid.time.series.2 %>%
  ...
  filter(retail_and_recreation_percent_change_from_baseline <= 240 & 
           grocery_and_pharmacy_percent_change_from_baseline <= 240 & 
           transit_stations_percent_change_from_baseline <= 240 & 
           workplaces_percent_change_from_baseline <= 240)
...
covid.small.county <- c(unique(covid.final[covid.final$growth.rate.avg > 1.3,]$county))
...
covid.final %>%
  subset(county %in% covid.large.county) %>% 
  subset(!county %in% c("New York Suffolk County", "New York Queens County", "California Los Angeles County", "Pennsylvania Philadelphia County", "Texas Bexar County")) %>%
  group_by(county) %>%
...
# ("New York Suffolk County", "New York Queens County", "California Los Angeles County", "Pennsylvania Philadelphia County", "Texas Bextar County"))
```
