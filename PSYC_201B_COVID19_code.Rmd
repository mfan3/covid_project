...
```{r}
covid.time.series %>% filter(state == "New Jersey")

unique(covid.time.series$county)
```
...
```{r}
residential.graph <- 
    ggplot(data=covid.replication,aes(x=mobility.active.country.res,y=retail.bottom.res)) + 
    ...
    theme_bw() +
    ggtitle("linear regression of retail residual on mobility average residual")
